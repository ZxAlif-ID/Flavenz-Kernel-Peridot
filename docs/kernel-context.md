...
6. **Fix Duck Detector build time drift** (2026-09-10): timestamp build kernel kini diturunkan dari `ro.build.date` ROM via input `rom_build_date` (default `2026-07-01`) — uname -a sinkron dengan system, anomali "build time drift / mismatch" hilang. Droidspaces **full requirement terkonfirmasi via flash test user** (diinjeksi ke AK3 Theettam, boot OK).
7. **Menunggu aksi user**: uji flash zip dari Releases (build berikutnya dengan timestamp ROM-synced); upload `recovery.log` baru ke `logs/` bila gagal.
